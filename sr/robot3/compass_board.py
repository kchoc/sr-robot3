...
class CompassBoard(Board):
    ...
    def get_board_type() -> str:
        ...
        :return: The literal string 'Compass'.
        """
        return 'Compass'
    ...
    def _get_valid_board(
        ...
        serial_port: str,
        initial_identity: Optional[BoardIdentity] = None,
    ) -> Optional[CompassBoard]:
        """
        Attempt to connect to an compass board and returning None if it fails identification.

        :param serial_port: The serial port to connect to.
        :param initial_identity: The identity of the board, as reported by the USB descriptor.

        :return: A CompassBoard object, or None if the board could not be identified.
        """
        try:
            ...
            if initial_identity is not None:
                if initial_identity.board_type == 'manual':
                    logger.warning(
                        f"Manually specified compass board at port {serial_port!r}, "
                        "could not be identified. Ignoring this device")
                elif initial_identity.manufacturer == 'sbot_simulator':
                    logger.warning(
                        f"Simulator specified compass board at port {serial_port!r}, "
                        "could not be identified. Ignoring this device")
                return None

            logger.warning(
                f"Found compass board-like serial port at {serial_port!r}, "
                "but it could not be identified. Ignoring this device")
            return None
        return board
    
    @classmethod
    def _get_simulator_boards(cls) -> MappingProxyType[str, CompassBoard]:
        """
        ...
        for board_info in get_simulator_boards('CompassBoard'):
    ...
    def _get_supported_boards(
    ...
    ) -> MappingProxyType[str, CompassBoard]:
        """
        Find all connected compass boards.
        ...
        :return: A mapping of serial numbers to compass boards.
        """
        if IN_SIMULATOR:
    ...
    def heading(self) -> float:
        ...
        """
        response = self._serial.query('COMP:READ?')
